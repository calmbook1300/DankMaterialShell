...
// XML file : internal/proto/xml/wlr-output-management-unstable-v1.xml
//
...
func (i *ZwlrOutputManagerV1) Dispatch(opcode uint32, fd int, data []byte) {
	...
	case 0:
		...
		l := 0
		headID := client.Uint32(data[l : l+4])
		if head, ok := i.Context().GetProxy(headID).(*ZwlrOutputHeadV1); ok && !head.IsZombie() {
			e.Head = head
		} else if headID != 0 {
			head := &ZwlrOutputHeadV1{}
			i.Context().RegisterWithID(head, headID)
			e.Head = head
		}
		l += 4
...
func (i *ZwlrOutputHeadV1) Dispatch(opcode uint32, fd int, data []byte) {
	...
	case 3:
		...
		l := 0
		modeID := client.Uint32(data[l : l+4])
		if mode, ok := i.Context().GetProxy(modeID).(*ZwlrOutputModeV1); ok && !mode.IsZombie() {
			e.Mode = mode
		} else if modeID != 0 {
			mode := &ZwlrOutputModeV1{}
			i.Context().RegisterWithID(mode, modeID)
			e.Mode = mode
		}
		l += 4
